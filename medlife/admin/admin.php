							...
							<div class="card-body p-0">
								<div class="table-responsive">
									<table class="table mb-0">
										...
										<?php
					$date= date('y-m-d');
					$sql1 = "SELECT A.id, D.first_name,D.last_name,P.fname,P.lname,P.address,P.ssn,T.fromm,T.too,A.date,A.time_id, A.status
					FROM appointments AS A, patients AS P, timing AS T , doctors AS D WHERE A.patient_ssn=P.ssn
					 AND A.time_id=T.id AND A.dr_id=D.id AND A.date>='$date' AND A.status=1 order by A.date,A.time_id DESC;";
					$stmt1 = $connection->prepare($sql1);
					$stmt1->execute();
					 $result = $stmt1->get_result();
